...
//SSL_HASH_ALGORITHM_ID__reserved,
//SSL_HASH_ALGORITHM_ID_INTRINSIC,
...
typedef enum {
  ...
  SSL_SIGNATURE_SHA224_WITH_DSA            = SSL_MK_SIGNATURE_ALG(SSL_HASH_ALGORITHM_ID_SHA224,    CRYPTO_X509_PK_ALGORITHM_ID_DSA),
  SSL_SIGNATURE_SHA256_WITH_DSA            = SSL_MK_SIGNATURE_ALG(SSL_HASH_ALGORITHM_ID_SHA256,    CRYPTO_X509_PK_ALGORITHM_ID_DSA),
  SSL_SIGNATURE_SHA384_WITH_DSA            = SSL_MK_SIGNATURE_ALG(SSL_HASH_ALGORITHM_ID_SHA384,    CRYPTO_X509_PK_ALGORITHM_ID_DSA),
  SSL_SIGNATURE_SHA512_WITH_DSA            = SSL_MK_SIGNATURE_ALG(SSL_HASH_ALGORITHM_ID_SHA512,    CRYPTO_X509_PK_ALGORITHM_ID_DSA),
  /* ECDSA */
  ...
  /* EdDSA */
//SSL_SIGNATURE_INTRINSIC_ED25519          = SSL_MK_SIGNATURE_ALG(SSL_HASH_ALGORITHM_ID_INTRINSIC, CRYPTO_X509_PK_ALGORITHM_ID_ED25519),
//SSL_SIGNATURE_INTRINSIC_ED448            = SSL_MK_SIGNATURE_ALG(SSL_HASH_ALGORITHM_ID_INTRINSIC, CRYPTO_X509_PK_ALGORITHM_ID_ED448),
  //
...
typedef struct {
  U8               * pData;
  unsigned           DataLen;
  SSL_CONTENT_TYPE   Type;
} SSL_FLIGHT_MESSAGE;

typedef struct {
  SSL_FLIGHT_MESSAGE aFlight[8];
  unsigned           Cnt;
} SSL_FLIGHT;
...
typedef enum {
  ...
  SSL_CLIENT_WAIT_HELLO_VERIFY_REQUEST,
  SSL_CLIENT_SEND_CLIENT_HELLO_AGAIN,
...
struct SSL_SESSION_tag {
  ...
  //
  // TLS version we and the peer will be using and the
  // client TLS version to prevent version rollback attacks.
  //
  U16 ProtocolVersion;
  ...
  // DTLS-related data.
  //
  U16  RxDtlsEpoch;           // In DTLSPlaintext, incremented on every cipher state change; always zero for TLS protocol
  U16  TxDtlsEpoch;           // In DTLSPlaintext, incremented on every cipher state change; always zero for TLS protocol
  U16  RxHandshakeMsgSeq;     // For receiving Handshake.message_seq
  U16  TxHandshakeMsgSeq;     // For transmitting Handshake.message_seq
  U8 * pHandshakeMsg;         // Handshake message
  U32  HandshakeMsgLen;       // Handshake message length taken from handshake header.
  U32  HandshakeMsgCurLen;    // Handshake message length as being reassembled from DTLS fragments.
  U8 * pDatagram;             // Buffer for incoming TLS fragment
  U16  DatagramCapacity;      // Maximum size of datagram buffer
  U16  DatagramLen;           // Size of received datagram
  U16  DatagramIdx;           // Index into buffer of received datagram
  //
  ...
  unsigned   SuiteCnt;
  //
  // Server name for SNI and for subject identification.
  //
  const char *sServerName;
  //
  // Server IPv4 address for subject identification, network byte order.
  //
  U8 aServerIp[4];
  //
  ...
  //
  CRYPTO_TLV RenegotiationClientData;          // "renegotiated_connection" field of the renegotiation information
  int        SawEmptyRenegotiationInfoSCSV;    // Cipher list contains a EMPTY_RENEGOTIATION_INFO_SCSV
  int        SecureRenegotiation;              // "secure_renegotiation" flag from RFC 5746
  //
  ...
  U8         aCookie[32];  // DTLS cookie
  U8         CookieLen;    // 0 -> no cookie, 32 -> have cookie
  //
