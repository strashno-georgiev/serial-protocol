...
*/
#define SSL_VERSION               26400  // Format is "Mmmrr" so, for example, 25401 corresponds to version 2.54a.
...
#define SSL_PROTOCOL_ID_DTLS_1v0  0xFEFF      // DTLS 1.0
#define SSL_PROTOCOL_ID_DTLS_1v2  0xFEFD      // DTLS 1.2
...
#define SSL_LOG_GENERAL         (1uL << 16)  // Log generally useful information.
...
#define SSL_SUITE_ID_AES_128_GCM_SHA256                         0x1301 // [RFC8446]
#define SSL_SUITE_ID_AES_256_GCM_SHA384                         0x1302 // [RFC8446]
#define SSL_SUITE_ID_CHACHA20_POLY1305_SHA256                   0x1303 // [RFC8446]
#define SSL_SUITE_ID_AES_128_CCM_SHA256                         0x1304 // [RFC8446]
#define SSL_SUITE_ID_AES_128_CCM_8_SHA256                       0x1305 // [RFC8446]
...
*/
#define SSL_SESSION_FLAG_REQUEST_CERTIFICATE                  0x0001u  // Set by user:  emSSL Server must request a client certificate for mutual authentication.
#define SSL_SESSION_FLAG_CERTIFICATE_RECEIVED                 0x0002u  // Set by emSSL: Client provided a valid certificate to emSSL server.
#define SSL_SESSION_FLAG_CERTIFICATE_REQUEST_RECEIVED         0x0004u  // Set by emSSL: Server requested certificate, if we have one.
#define SSL_SESSION_FLAG_CERTIFICATE_SENT                     0x0008u  // Set by emSSL: Client sent a valid certificate as we have one.
#define SSL_SESSION_FLAG_REQUEST_RESUME_SESSION_ID            0x0010u  // Set by emSSL: Indicates session resumption is requested by session ID.
#define SSL_SESSION_FLAG_REQUEST_RESUME_SESSION_TICKET        0x0020u  // Set by emSSL: Indicates session resumption is requested by session ticket.
#define SSL_SESSION_FLAG_REQUEST_RESUME_CRITICAL              0x0040u  // Set by user:  If session resumption is requested, it is critical that a session is resumed (no new session offered).  If not, fail.
#define SSL_SESSION_FLAG_RESUME_GRANTED                       0x0080u  // Set by emSSL: After connection completes, indicates that a session is successfully resumed.
#define SSL_SESSION_FLAG_DISABLE_SESSION_TICKET               0x0100u  // Set by user:  Do not use session ticket even if configured.
#define SSL_SESSION_FLAG_SESSION_TICKET_EXPECTED              0x0200u  // Set by emSSL: The client knows the server supports session tickets.
#define SSL_SESSION_FLAG_SESSION_TICKET_EXTENSION_ADVERTISED  0x0400u  // Set by emSSL: The server indicates its willingness to issue session tickets.
#define SSL_SESSION_FLAG_SESSION_TICKET_RECEIVED              0x0800u  // Set by emSSL: The client has received a session ticket.
#define SSL_SESSION_FLAG_DISABLE_RSA_PMS_VERSION_CHECK        0x1000u  // Set by user:  Disable TLS 1.0 version check [TLS1v2 https://tools.ietf.org/html/rfc5246#section-7.4.7.1].
#define SSL_SESSION_FLAG_REQUEST_PREVENT_FALLBACK             0x2000u  // Set by user:  Request Fallback SCSV signaling cipher suite [RFC7507 https://tools.ietf.org/html/rfc7507].
#define SSL_SESSION_FLAG_PREFER_SERVER_ORDER                  0x4000u  // Set by user:  Prefer the server's cipher suite order to the client's preferred order.
#define SSL_SESSION_FLAG_REQUIRE_STRICT_TLS_CLOSE             0x8000u  // Set by user:  Require a TLS close-notify from peer to close the session.
...
*    <  0 - Error.
*/
typedef int SSL_SEND_FUNC(int Socket, const char *pData, int Len, int Flags);
...
*    <  0 - Error.
*/
typedef int SSL_RECV_FUNC(int Socket, char* pData, int Len, int Flags);         // Receive 8-bit clean data from transport
...
typedef struct SSL_TRANSPORT_API_tag {
  SSL_SEND_FUNC     *pfSend;        // Send data to network
  SSL_RECV_FUNC     *pfReceive;     // Receive data from network
  SSL_GET_TIME_FUNC *pfGetTime;     // Get current time
  SSL_IS_UDP_FUNC   *pfIsUDP;       // Inquire if transport is UDP (datagram)
} SSL_TRANSPORT_API;
...
int               SSL_ROOT_CERTIFICATE_LoadDER  (SSL_ROOT_CERTIFICATE *pCert, const U8 *pData, unsigned DataLen);
int               SSL_ROOT_CERTIFICATE_AddDER   (const U8 *pData, unsigned DataLen);
...
void              SSL_MEM_SetContext            (SEGGER_MEM_CONTEXT *pMem);
...
void              SSL_OS_Sleep                  (unsigned Duration);
...
*       Utility functions
*/
unsigned          SSL_StrLen                    (const char *sText);
