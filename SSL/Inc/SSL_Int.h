...
#define SSL_ALLOC_TYPE(LVALUE, T)    /*lint -e{717} */     do { if (((LVALUE) = (T *)SSL_MEM_Alloc(sizeof(T))) == 0)           { (void)SSL_LOG_ERR(SSL_ERROR_OUT_OF_MEMORY); SSL_RAISE(SSL_ERROR_OUT_OF_MEMORY); } } while (0)
#define SSL_ALLOC_VPTR(LVALUE, SIZE) /*lint -e{717} */     do { if (((LVALUE) = SSL_MEM_Alloc(SIZE)) == 0 && (SIZE) > 0)       { (void)SSL_LOG_ERR(SSL_ERROR_OUT_OF_MEMORY); SSL_RAISE(SSL_ERROR_OUT_OF_MEMORY); } } while (0)
#define SSL_ALLOC_U8(LVALUE, SIZE)   /*lint -e{717} */     do { if (((LVALUE) = (U8 *)SSL_MEM_Alloc(SIZE)) == 0 && (SIZE) > 0) { (void)SSL_LOG_ERR(SSL_ERROR_OUT_OF_MEMORY); SSL_RAISE(SSL_ERROR_OUT_OF_MEMORY); } } while (0)
...
// For older clients that dipped into internals
#define SSL_MEM_ConfigUser SSL_MEM_SetContext
...
*/
const char                  * SSL_DecodeRecordProtocol        (unsigned Type);
const char                  * SSL_SIGNATURE_ALGORITHM_GetName (SSL_SIGNATURE_ALGORITHM_ID Id);
...
/*********************************************************************
*
*       Root certificate functions
*/
void                SSL_ROOT_CERTIFICATE_Exit(void);
