...
Purpose : Sample program for embOS & TCP/IP & SNMP agent.
...
//
  ...
  #define TRAP_INFORM_0_RETRIES    5                                         // Retries for INFORM messages, ignored for other types.
#endif
...
//
  ...
  #define TRAP_INFORM_1_RETRIES    5                                         // Retries for INFORM messages, ignored for other types.
#endif
...
static OS_STACKPTR int _SNMP_AgentMessageStack[2048/sizeof(int)];           // Stack of the SNMP agent message handler.
static OS_TASK         _SNMP_AgentMessageTCB;                               // Task-Control-Block of SNMP message handler.

//
// In and out buffers for SNMP.
//
static char _acSNMP_RequestBuffer[MAX_SNMP_MESSAGE_SIZE];
static char _acSNMP_ResponseBuffer[MAX_SNMP_MESSAGE_SIZE];
static char _acSNMP_InformAckBuffer[MAX_SNMP_MESSAGE_SIZE];
...
static U8                    _abTrapInformSendBuffer[1472];     // Buffer where the complete message of headers and Varbinds is assembled.
...
static void* _SNMP_AllocSendBuffer(void* pUserContext, U8** ppBuffer, U32 NumBytes, U8 IPAddrLen) {
  ...
  if (NumBytes <= sizeof(_abTrapInformSendBuffer)) {
    p         = (void*)&_abTrapInformSendBuffer[0];
    *ppBuffer =        &_abTrapInformSendBuffer[0];
  } else {
    p = NULL;
  }
  return p;
...
static void _SNMP_FreeSendBuffer(void* pUserContext, void* p, char SendCalled, int r) {
  IP_USE_PARA(pUserContext);
  IP_USE_PARA(p);
  IP_USE_PARA(SendCalled);
  IP_USE_PARA(r);

  //
  // This sample uses a static buffer and therefore does not need to free it.
  //
}
...
static int _SNMP_SendTrapInform(void* pContext, void* pUserContext, void* hBuffer, const U8* pData, U32 NumBytes, U8* pIPAddr, U16 Port, U8 IPAddrLen) {
  ...
  memcpy(&Addr.sin_addr.s_addr, pIPAddr, IPAddrLen);
  return sendto(hSock, (const char*)pData, NumBytes, 0, (struct sockaddr*)&Addr, sizeof(Addr));
}
...
*
*  Parameters
*    hSockTrapInform: Socket to use for sending.
*
*  Additional information
...
*/
static void _SNMP_AGENT_SendBootTrap(int hSockTrapInform) {
  U32      IPAddr;
  I32      Timeout;
  unsigned u;
  int      r;
  ...
  for (u = 0; u < SEGGER_COUNTOF(_aTrapInformConfig); u++) {
    r = IP_SNMP_AGENT_SendTrapInform((void*)hSockTrapInform, &_TrapInformVarbindContext, &_aTrapInformConfig[u]);
    if        (r <  0) {
      //
      // Error.
      //
      ...
      // that work on the receiver has been done, either by success
      // or with an error.
      //
      ...
      // send one TRAP/INFORM message after another as _SNMP_AllocSendBuffer()
      // can only handle one packet to send at the same time.
      // If _SNMP_AllocSendBuffer() returns allocated memory or used multiple
      // buffers, the following steps can be done at any other time.
      //
      while (1) {
        r = IP_SNMP_AGENT_CheckInformStatus(&_aTrapInformConfig[u]);
...
/*********************************************************************
*
*       _SNMP_AGENT_MessageTask()
*
*  Function description
*    Server task that receives incoming INFORM responses and handles
*    SNMP request messages and sending back to them.
*
*  Parameters
*    pContext: Pointer to structure with sockets to be used.
*/
static void _SNMP_AGENT_MessageTask(void* pContext) {
         IP_fd_set   ReadFds;
  struct sockaddr_in Addr;
         int         AddrLen;
         int         MessageLen;
  ...
  hSockMessage    = ((SNMP_AGENT_SOCKETS*)pContext)->hSockMessage;
  hSockTrapInform = ((SNMP_AGENT_SOCKETS*)pContext)->hSockTrapInform;
  //
  // Wait for incoming SNMP messages.
  //
  while (1) {
    IP_FD_ZERO(&ReadFds);                  // Clear the set.
    IP_FD_SET(hSockMessage   , &ReadFds);  // Add message socket to the set.
    IP_FD_SET(hSockTrapInform, &ReadFds);  // Add TRAP/INFORM socket to the set.
    r = select(&ReadFds, NULL, NULL, -1);  // Wait for activity (data received).
    if (r <= 0) {
      continue;
    }
    //
    // Handle INFORM response. This is done first as this is very fast.
    //
    if (IP_FD_ISSET(hSockTrapInform, &ReadFds)) {
      AddrLen = sizeof(Addr);
      //
      // Receive SNMP INFORM message.
      //
      MessageLen = recvfrom(hSockTrapInform, &_acSNMP_InformAckBuffer[0], sizeof(_acSNMP_InformAckBuffer), 0, (struct sockaddr*)&Addr, &AddrLen);  // Senders addr. and port will be stored in Addr.
      //
      // Check that we really received a message and not a socket error.
      //
      if (MessageLen > 0) {
        //
        // Check the received message to be a valid response for an INFORM sent.
        //
        IP_SNMP_AGENT_ProcessInformResponse((const U8*)&_acSNMP_InformAckBuffer[0], MessageLen);
      }
    }
    //
    // Handle SNMP requests.
    //
    if (IP_FD_ISSET(hSockMessage, &ReadFds)) {
      AddrLen = sizeof(Addr);
      //
      // Receive SNMP request.
      //
      MessageLen = recvfrom(hSockMessage, &_acSNMP_RequestBuffer[0], sizeof(_acSNMP_RequestBuffer), 0, (struct sockaddr*)&Addr, &AddrLen);  // Senders addr. and port will be stored in Addr.
      //
      // Check that we really received a message and not a socket error.
      //
      if (MessageLen > 0) {
        //
        // Process the received message. The response will be prepared in the output buffer
        // for sending back from where we received the SNMP message.
        //
        ResponseLen = IP_SNMP_AGENT_ProcessRequest((const U8*)&_acSNMP_RequestBuffer[0], MessageLen, (U8*)&_acSNMP_ResponseBuffer[0], sizeof(_acSNMP_ResponseBuffer), NULL);
        //
        // Send back a response if we have to.
        //
        if (ResponseLen > 0) {
          sendto(hSockMessage, &_acSNMP_ResponseBuffer[0], ResponseLen, 0, (struct sockaddr*)&Addr, sizeof(Addr));
        }
      }
    }
  }
...
*
*  Function description
*    Initializes and configures the SNMP agent and creates sockets
*    for message handling.
*
*  Parameters
*    pSockets: Struct with sockets to fill in for later usage.
*/
static void _SNMP_AGENT_InitConfig(SNMP_AGENT_SOCKETS* pSockets) {
  struct sockaddr_in Addr;
         int         hSock;
         int         r;

  //
  // Loop until we get a socket for message handling.
  //
  do {
    ...
    if (hSock != SOCKET_ERROR) {
      ...
      pSockets->hSockMessage  = hSock;
      break;
    }
    OS_Delay(100);  // Try again after delay.
  } while (1);
  //
  // Loop until we get a socket for sending TRAP/INFORM messages and receiving INFORM ACKs.
  //
  do {
    hSock = socket(AF_INET, SOCK_DGRAM, 0);
    if (hSock != SOCKET_ERROR) {
      ...
      memset(&Addr, 0, sizeof(Addr));
      Addr.sin_family            = AF_INET;
      Addr.sin_port              = htons(TRAP_INFORM_PORT);
      Addr.sin_addr.s_addr       = INADDR_ANY;
      bind(hSock, (struct sockaddr*)&Addr, sizeof(Addr));
      pSockets->hSockTrapInform  = hSock;
      break;
    }
    OS_Delay(100);  // Try again after delay.
  } while (1);
  ...
  //
  // Add a community string (without termination) and set permissions for it.
  //
  ...
  //
  // Add a second community string (without termination) and set permissions for it.
  //
...
void MainTask(void) {
  ...
  _SNMP_AGENT_InitConfig(&Sockets);                                                    // Initialize and configure the SNMP agent.
  ...
  //
  // Start SNMP agent task for message handling.
  //
  OS_CREATETASK_EX(&_SNMP_AgentMessageTCB, "SNMPa message", _SNMP_AGENT_MessageTask, TASK_PRIO_IP_SNMP_AGENT_MESSAGE, _SNMP_AgentMessageStack, (void*)&Sockets);
  //
  // Send boot trap to a list of managers to let them know we are alive.
  //
  _SNMP_AGENT_SendBootTrap(Sockets.hSockTrapInform);
  //
