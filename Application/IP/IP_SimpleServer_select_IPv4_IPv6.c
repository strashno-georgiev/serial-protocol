...
File    : IP_SimpleServer_select_IPv4_IPv6.c
...
*
* Parameter
*   IPProtVer - Protocol family which should be used (AF_INET or PF_INET6).
*   hSock     - Socket handle
...
static void _TelnetTask(void) {
  ...
  //
  // Try until we get a valid IPv6 parent socket.
  //
  while (1) {
  ...
  while (1) {
    ...
    if        (IP_FD_ISSET(hSockParent4, &ReadFds)) {
      ...
      //
      // Check for a new IPv6 connection.
      //
      hSockChild = accept(hSockParent6, (struct sockaddr*)&Addr, &AddrLen);
      if (hSockChild == SOCKET_ERROR) {
        continue;             // Error, try again.
      }
      IP_Logf_Application("New IPv6 client (%n) accepted.", Addr.sin6_addr.Union.aU8);
    }
