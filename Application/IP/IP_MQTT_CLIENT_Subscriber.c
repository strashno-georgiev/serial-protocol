...
static void _MQTT_Client(void) {
  ...
  do {
    ...
    TopicFilter.QoS          = IP_MQTT_CLIENT_FLAGS_PUBLISH_QOS1;
...
WaitForMessage:
        ...
        // The whole construct depends on the IP_MQTT_CLIENT_WaitForNextMessage not blocking forever.
        // In this sample the function returns as soon as the socket timeout is reached (RCV_TIMEOUT).
        //
        ...
        //
        r        = IP_MQTT_CLIENT_WaitForNextMessage(&_MQTTClient, &Topic, &TotalLen, _acTopic, sizeof(_acTopic));
        if ((r > 0) && (Topic == IP_MQTT_CLIENT_TYPES_PUBLISH)) {
          ...
          if (r < (int)sizeof(_acTopic)) {
            ...
            IP_MQTT_CLIENT_APP_LOG(("APP:   Topic  : %s", _acTopic));
          }
...
RetryOnTimeout:
              r = IP_MQTT_CLIENT_Recv(&_MQTTClient, _aPayload, v);
              if        (r == 0) {
              ...
              if (_MQTTClient.Version == 5) {
                IP_MQTT_CLIENT_APP_LOG(("APP:   Payload: %s", &_aPayload[1])); // This only works if the incoming packet has no MQTT properties.
              } else {
                IP_MQTT_CLIENT_APP_LOG(("APP:   Payload: %s", &_aPayload[0]));
              }
