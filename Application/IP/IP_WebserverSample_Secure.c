...
File    : IP_WebserverSample_Secure.c
Purpose : Sample program for embOS & TCP/IP and SSL.
*/
...
static void _WebServerChildTask(void *pContext) {
  ...
  r        = 0;
  pSession = (SSL_SESSION*)pContext;
  hSock    = (long)pSession->Socket;
  Opt      = 1;
  ...
  // session using SSL.
  //
  if (SSL_SESSION_Accept(pSession) >= 0) {
      ...
      if (_ConnectCnt < MAX_CONNECTIONS) {
        for (i = 0; i < MAX_CONNECTIONS; i++) {
          ...
          if (r == 0) {
            ...
            OS_CREATETASK_EX(&_aWebTasks[i], "Webserver Child", _WebServerChildTask, TASK_PRIO_WEBS_CHILD, _aWebStacks[i], (void *)&_aSSLSession[i]);
            _AddToConnectCnt(1);
            goto Next;
          }
        }
