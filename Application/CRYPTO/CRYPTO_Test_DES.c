...
File        : CRYPTO_Test_DES.c
Purpose     : Run all CRYPTO TDES self tests.
...
static void _RunTests(void) {
  ...
  SEGGER_SYS_IO_Printf("---------------------------------------------\n");
  //
  CRYPTO_TDES_ECB_CAVS_SelfTest(&SelfTestAPI);
  CRYPTO_TDES_CBC_CAVS_SelfTest(&SelfTestAPI);
}
...
void MainTask(void) {
  ...
  SEGGER_SYS_IO_Printf("DES Self-Test compiled " __DATE__ " " __TIME__ "\n\n");
  //
  CRYPTO_TDES_QueryInstall(&pHWAPI, &pSWAPI);
  if (pHWAPI == NULL && pSWAPI == NULL) {
    SEGGER_SYS_IO_Printf("No TDES cipher installed!\n");
    SEGGER_SYS_OS_Halt(100);
  }
  //
  _RunTests();
  //
  if (pHWAPI && pSWAPI) {
    SEGGER_SYS_IO_Printf("\nRerunning with TDES accelerator deactivated\n\n");
