...
File        : CRYPTO_Test_MD5.c
Purpose     : Run all MD5 self tests.
...
static void _RunTests(void) {
  ...
  SEGGER_SYS_IO_Printf("---------------------------------------------\n");
  //
  CRYPTO_MD5_RFC1321_SelfTest(&SelfTestAPI);
}
...
void MainTask(void) {
  ...
  SEGGER_SYS_IO_Printf("MD5 Self-Test compiled " __DATE__ " " __TIME__ "\n\n");
  //
  _RunTests();
  CRYPTO_MD5_QueryInstall(&pHWAPI, &pSWAPI);
  if (pHWAPI && pHWAPI != &CRYPTO_HASH_MD5_SW) {
    SEGGER_SYS_IO_Printf("\nRerunning with MD5 accelerator deactivated\n\n");
