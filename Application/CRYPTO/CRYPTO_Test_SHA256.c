...
File        : CRYPTO_Test_SHA256.c
Purpose     : Run all CRYPTO SHA-256 self tests.
...
static void _RunTests(void) {
  ...
  SEGGER_SYS_IO_Printf("---------------------------------------------\n");
  //
  CRYPTO_SHA256_FIPS180_SelfTest(&SelfTestAPI);
  CRYPTO_SHA256_CAVS_SelfTest   (&SelfTestAPI);
}
...
void MainTask(void) {
  ...
  SEGGER_SYS_IO_Printf("SHA-256 Self-Test compiled " __DATE__ " " __TIME__ "\n\n");
  //
  CRYPTO_SHA256_QueryInstall(&pHWAPI, &pSWAPI);
  if (pHWAPI == NULL && pSWAPI == NULL) {
    SEGGER_SYS_IO_Printf("No SHA256 hash algorithm installed!\n");
    SEGGER_SYS_OS_Halt(100);
  }
  //
  _RunTests();
  ...
  if (pHWAPI && pHWAPI != &CRYPTO_HASH_SHA256_SW) {
    SEGGER_SYS_IO_Printf("\nRerunning with SHA-256 accelerator deactivated\n\n");
