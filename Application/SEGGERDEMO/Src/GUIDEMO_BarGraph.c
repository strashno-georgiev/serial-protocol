...
*       _DrawDiagramAt
*/
static void _DrawDiagramAt(GUI_MEMDEV_Handle hMem, int xPos, int yPos, int * py, int xBlend, GUI_MEMDEV_Handle hMemTrans) {
  GUI_MEMDEV_Handle hMemOld;
  GUI_RECT          Rect;
  int               IndexBmBar;
  int               ySizeBar;
...
static void _DemoBarGraph(void) {
  ...
  // Create MEMDEV
  //
  hMem = GUI_MEMDEV_CreateFixed32(xPosGraph, yPosGraph, GRAPH_WIDTH + 7 + 1, GRAPH_HEIGHT + 1);
  if (hMem == 0) {
    return;
  }
  hMemTrans = GUI_MEMDEV_CreateFixed32(xPosGraph, yPosGraph, GRAPH_WIDTH + 7 + 1, GRAPH_HEIGHT + 1);
  if (hMemTrans == 0) {
  ...
  GUI_FillRoundedRect(xPosGraph, yPosGraph, xPosGraph + GRAPH_WIDTH + 4 , yPosGraph + GRAPH_HEIGHT, 4);
  GUI_MEMDEV_Select(0);
  ...
  do {
    ...
    _DrawDiagramAt(hMem, xPosGraph, yPosGraph, ayCur, Blend, hMemTrans);
    Blend += AddBlend;
