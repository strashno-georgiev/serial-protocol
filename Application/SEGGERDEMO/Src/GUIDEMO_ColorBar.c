...
void GUIDEMO_ColorBar(void) {
  ...
  GUIDEMO_ConfigureDemo(acTitle, pDesc, GUIDEMO_SHOW_CURSOR | GUIDEMO_SHOW_CONTROL);
  GUIDEMO_DrawBk();
  //
  // Heading
  //
  ...
  while (((GUIDEMO_GetTime() - TimeStart) < TIME_RUN) && (GUIDEMO_CheckCancel() == 0)) {
    ...
    ColorStartBlack = GUI_MAKE_COLOR(0x000000 + 0x010101 * Index);
    ColorStartWhite = GUI_MAKE_COLOR(0xFFFFFF - ColorStartBlack);
