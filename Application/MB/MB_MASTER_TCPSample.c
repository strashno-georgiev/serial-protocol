  ...
  if (hSock == SOCKET_ERROR) {
    ...
    for (;;) {
      ...
      if (SoError != IP_ERR_IN_PROGRESS) {
        r = MB_ERR_CONNECT;                              // Unable to connect to slave.
        break;
      }
      t = IP_OS_GetTime32() - t0;
      if (t >= (int)Timeout) {
        r = MB_ERR_CONNECT_TIMEOUT;                      // Unable to connect to slave within timeout.
        break;
      }
      OS_Delay(1);                                       // Give lower prior tasks some time.
    }
    if (r != 0) {                                        // Connect error ?
      closesocket(hSock);                                // ==> Close socket as we do not need it anymore.
    }
  }
  return r;
}
...
static int _Recv(MB_IFACE_CONFIG_IP *pConfig, U8 *pData, U32 NumBytes, U32 Timeout) {
  ...
  //
  // This code is identical to the master counterpart and is therefore shared.
  // A slave however does not have a timeout given by the stack/configuration but
  // might still have a receive timeout set for the socket somewhere.
  // Therefore we check if the receive timeout is 0 (disabled) so far and only in
  // this case set a timeout (for the master).
  //
  // We check for timeout disabled by treating < 0 as disabled. This does not have
  // to be true for each and every TCP/IP stack out there but should fit the majority
  // of them as having 0 as valid timeout enabled value makes only little sense.
  //
  r = getsockopt(hSock, SOL_SOCKET, SO_RCVTIMEO, &OrgTimeout, sizeof(OrgTimeout));
  if (r != SOCKET_ERROR) {                                                    // Can we retrieve the currently set timeout ?
    if ((OrgTimeout <= 0) && (Timeout != 0u)) {                               // ==> No timeout set so far AND we want to set a timeout (master only) ?
      setsockopt(hSock, SOL_SOCKET, SO_RCVTIMEO, &Timeout, sizeof(Timeout));  // ====> Set OUR timeout.
    }
  }
