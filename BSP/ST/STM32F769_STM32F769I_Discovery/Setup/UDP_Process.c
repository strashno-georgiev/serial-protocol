...
void UDP_Process_Send1(char c) {
  ...
  if (PeerAddr != 0u) {
    ...
    //
    pOutPacket = IP_UDP_Alloc(NumBytes + PROTOCOL_SIZE + Offset);
    if (pOutPacket) {
      //
      ...
      if (NumBytes >= 0xF0) {
        pOutPacket->pData[2] = (NumBytes >> 8) | 0xF0;
        pOutPacket->pData[3] = NumBytes & 0xFF;
      } else {
        pOutPacket->pData[2] = NumBytes;
      }

      for (i = 0; i < NumBytes + 2 + Offset; i++) {
        pOutPacket->pData[i + 3 + Offset] = OS_COM_GetNextChar();
      }
