...
function GetObjectName(Address) {
  ...
  p = Debug.evaluate("OS_Global.pObjNameRoot");
  if (p == undefined || p == 0) {
    return "";
  }
...
function GetTaskName(Address) {
  ...
  Name = Debug.evaluate("(char*)(*(OS_TASK*)" + Address + ").sName")  // Since V5.16.0: OS_TASK.sName
      || Debug.evaluate("(char*)(*(OS_TASK*)" + Address + ").Name")   // Until V5.14.0: OS_TASK.Name
      || "n.a.";                                                      // If pointer is zero or not available, set to "n.a."
  return Name;
}
...
function UpdateTimers(Window) {
  ...
  for (pTimer = OS_Global.pTimer; (pTimer != undefined) && (pTimer != 0); pTimer = Debug.evaluate("(*(OS_TIMER*)" + pTimer + ").pNext")) {
    ...
    if (Timer == undefined) {
      ...
      //
      Hook          = Timer.Timer.pfTimerRoutine;
...
function UpdateMailboxes(Window) {
  ...
  for (pMailbox = Debug.evaluate("OS_Global.pMailboxRoot"); (pMailbox != undefined) && (pMailbox != 0); pMailbox = Debug.evaluate("(OS_MAILBOX*)(*(OS_MAILBOX*)" + pMailbox + ").pNext")) {
...
function UpdateQueues(Window) {
  ...
  for (pQueue = Debug.evaluate("OS_Global.pQueueRoot"); (pQueue != undefined) && (pQueue != 0); pQueue = Debug.evaluate("(*" + CastQueue + pQueue + ").pNext")) {
...
function UpdateMemoryPools(Window) {
  ...
  for (pMemPool = Debug.evaluate("OS_Global.pMEMFRoot"); (pMemPool != undefined) && (pMemPool != 0); pMemPool = Debug.evaluate("(*" + CastMEMF + pMemPool + ").pNext")) {
...
function UpdateEventObjects(Window) {
  ...
  for (pEvent = Debug.evaluate("OS_Global.pEventRoot"); (pEvent != undefined) && (pEvent != 0); pEvent = Debug.evaluate("(*(OS_EVENT*)" + pEvent + ").pNext")) {
    //
    // Read information from queue structure
    //
...
function UpdateWatchDogs(Window) {
  ...
  for (pWatchdog = Debug.evaluate("OS_Global.pWDRoot"); (pWatchdog != undefined) && (pWatchdog != 0); pWatchdog = Debug.evaluate("(*(OS_WD*)" + pWatchdog + ").pNext")) {
...
function UpdateSystemInformation(Window) {
  ...
  // Read information
  //
  Status       = Debug.evaluate("OS_Global.Status");
...
function UpdateRWLock(Window) {
  ...
  for (pRWLock = Debug.evaluate("OS_Global.pRWLockRoot"); (pRWLock != undefined) && (pRWLock != 0); pRWLock = Debug.evaluate("(*(OS_RWLOCK*)" + pRWLock + ").pNext")) {
...
function update() {
  ...
  //
  if (Debug.evaluate("OS_Global.pRSemaRoot") != undefined) {
    CastMutex  = "(OS_RSEMA*)";
    CastSema   = "(OS_CSEMA*)";
    CastQueue  = "(OS_Q*)";
    CastMEMF   = "(OS_MEMF*)";
    ...
    if (OS_TASK.StackSize != undefined) {
      ...
      if (OS_TASK.StackSize > StackCheckLimit) {
        ...
        if (TargetInterface.findByte) {
          ...
          var abStack = TargetInterface.peekBytes(OS_TASK.pStackBase, OS_TASK.StackSize);
