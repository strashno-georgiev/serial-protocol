...
#define CRYPTO_POSIT(C, X)                    /*lint -e{717,801,9036} */ do { if (!(C)) { Status = X; goto Finally; } } while (0)
#define CRYPTO_RAISE(X)                       /*lint -e{717,801}      */ do { Status = (X); goto Finally; } while (0)
#define CRYPTO_ALLOC(LVALUE, PMEM, SIZE)      /*lint -e{717}          */ do { if (((LVALUE) = SEGGER_MEM_Alloc(pMem, SIZE)) == 0 && (SIZE) > 0)     { CRYPTO_RAISE(CRYPTO_ERROR_OUT_OF_MEMORY); } } while (0)
...
extern const U8  CRYPTO__U8_0x01_x16            [16];
extern const U8  CRYPTO__U8_0x02_x16            [16];
extern const U8  CRYPTO__U8_0x03_x16            [16];
...
*/
void     CRYPTO_CIPHER_IDEA_64_InitEncrypt              (void *pSelf, const U8 *pKey);
void     CRYPTO_CIPHER_IDEA_64_InitDecrypt              (void *pSelf, const U8 *pKey);
void     CRYPTO_CIPHER_IDEA_192_InitEncrypt             (void *pSelf, const U8 *pKey);
void     CRYPTO_CIPHER_IDEA_192_InitDecrypt             (void *pSelf, const U8 *pKey);
//
...
*/
void     CRYPTO_CIPHER_PRESENT_0_InitEncrypt            (void *pContext, const U8 *pKey);
void     CRYPTO_CIPHER_PRESENT_0_InitDecrypt            (void *pContext, const U8 *pKey);
...
extern void *CRYPTO_HW_SAML_TRNG;
extern void *CRYPTO_HW_RT10xx_DCP;
extern void *CRYPTO_HW_RT10xx_TRNG;
