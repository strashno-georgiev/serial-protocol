...
if exist "JLink.exe" (
  JLink.exe -device "%_DEVICE_%" -CommanderScript CommandFile_GUI_NormalMapping.jlink
  goto END
) else (
...
%_JLINKEXE% -device "%_DEVICE_%" -CommanderScript CommandFile_GUI_NormalMapping.jlink
